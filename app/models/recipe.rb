...
### INDEX SORT
  ...
  def self.search(search)
    if search.present?
        Recipe.includes(:ingredients).includes(:tags).where('lower(ingredients.name) LIKE ? OR lower(tags.name) LIKE ? OR lower(recipes.name) LIKE ? OR lower(recipes.description) LIKE ?' , "%#{search.downcase}%", "%#{search.downcase}%", "%#{search.downcase}%", "%#{search.downcase}%").references(:ingredients).references(:tags).order("recipes.created_at DESC").distinct()
    else
      Recipe.all
    end
  end
